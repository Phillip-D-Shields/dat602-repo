-- Trolls Gem Hunt Game Attempt

-- code shared with me and hacked into something else
----------------------------------------------------------------------------------
-- Database Setup
----------------------------------------------------------------------------------

DROP DATABASE IF EXISTS trollGameDatabase;
BEGIN;
CREATE DATABASE trollGameDatabase;
BEGIN;
USE trollGameDatabase;
...
DROP TABLE IF EXISTS tblRuby;
DROP TABLE IF EXISTS tblTroll;
...
CREATE TABLE tblTroll (
TrollName varchar(10) NOT NULL,
TileColour varchar(10) NOT NULL,
PRIMARY KEY (TrollName)
);

CREATE TABLE tblRuby (
RubyType varchar(10) NOT NULL,
Points tinyint NOT NULL,
PRIMARY KEY (RubyType)
);
...
CONSTRAINT FK_TrollName_Play FOREIGN KEY (TrollName) REFERENCES tblTroll(TrollName),
...
RubyType varchar(10) NOT NULL,
PRIMARY KEY (ItemID),
CONSTRAINT FK_GemType_Item FOREIGN KEY (RubyType) REFERENCES tblGem(RubyType)
);
...
BEGIN
	...
	WHERE TrollName = pCharacterName;
END
//
DELIMITER ;
...
BEGIN
	SELECT TrollName AS 'Character Name'
 	FROM tblCharacter
...
BEGIN
	SELECT RubyType AS 'Gem', Points AS 'Gem Points'
 	FROM tblGem
	WHERE RubyType = pGemType;
END
//
...
BEGIN
	SELECT GameID AS 'Game Ref', TrollName AS 'Character Name', PlayerID AS 'Player Ref', PlayScore AS 'Game Score'
 	FROM tblPlay
	WHERE TrollName = pCharacterName AND GameID = pGameID;
END
//
DELIMITER ;
...
BEGIN
	SELECT ItemID AS 'Item Ref', RubyType AS 'Gem'
 	FROM tblItem
	WHERE ItemID = pItemID;
END
//
